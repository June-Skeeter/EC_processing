    ...
    def __post_init__(self):
        if self.UID is None:
            if not type(self).__name__.endswith('Configuration'):
                self.logWarning('do I Need UID?')
                # self.formatUID('dataSourceID')
        else:
            self.logError('Implement better config flag')
        if not type(self).__name__.endswith('Configuration'):
...
headerText = 'Models not yet implemented'
@dataclass(kw_only=True)
class modelConfiguration(configCommon,dataSource):
    header: str = field(default=headerText,repr=False,init=False) # YAML header, must be treated differently
    measurementType: str = 'Model'
...
headerText = '???'
@dataclass(kw_only=True)
class measurementConfiguration(configCommon,dataSource):
    header: str = field(default=headerText,repr=False,init=False) # YAML header, must be treated differently
...
#     )
    ...
    def __post_init__(self):
        ...
        self.configName = f"{self.measurementType}_{self.sourceID}_sourceConfig.yml"
    ...
    def formatDataLogger(self):
        if type(self.dataLogger) is str:
            if hasattr(dataLoggers,self.dataLogger):
                ...
                self.logError(f'Could not format data logger: {self.dataLogger}')
            ...
            self.logMessage(f'Logger not found, parsing as generic logger {self.dataLogger}')
